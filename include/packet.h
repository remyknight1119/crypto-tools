                                           ...
                                           size_t num)
{
    if (PACKET_remaining(pkt) != num)
        return 0;
    return memcmp(pkt->curr, ptr, num) == 0;
}
 ...
 * If |*data| is not NULL, the old data is free'd.
...
static inline int PACKET_strndup(const PACKET *pkt, char **data)
{
    ...
    *data = strndup((const char *)pkt->curr, PACKET_remaining(pkt));
    return (*data != NULL);
}
